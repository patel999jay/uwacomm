...
def demo_mode3_multi_vehicle():
    ...
    encoded = encode_with_routing(
        status, source_id=3, dest_id=0, priority=2, ack_requested=True  # Topside  # High priority
    )
    ...
    ack = CommandAck(
        acked_msg_id=10, ack_status=0, error_code=0  # VehicleStatus message ID  # Success
    )

    ack_encoded = encode_with_routing(ack, source_id=0, dest_id=3, priority=3)  # Urgent (ACK)
    ...
    cmd_encoded = encode_with_routing(
        waypoint, source_id=0, dest_id=1, priority=2, ack_requested=True
    )

    routing, cmd = decode_with_routing(WaypointCommand, cmd_encoded)
    print("   Vehicle 1 received waypoint:")
...
def demo_broadcast_pattern():
    ...
    broadcast_msg = encode_with_routing(
        formation, source_id=1, dest_id=255, priority=2  # 255 = broadcast to all
    )

    print("Lead Vehicle (ID=1) broadcasts:")
    print("  Message: FormationUpdate")
    print("  Destination: 255 (BROADCAST)")
    print("  Formation type: Grid")
...
def bandwidth_summary():
    ...
    print("VehicleStatus message:")
    print(f"  Mode 1 (Point-to-Point):  {len(mode1):3d} bytes")
    print(
        f"  Mode 2 (Self-Describing): {len(mode2):3d} bytes  (+{len(mode2)-len(mode1)} bytes for ID)"
    )
    print(
        f"  Mode 3 (Multi-Vehicle):   {len(mode3):3d} bytes  (+{len(mode3)-len(mode1)} bytes for routing+ID)"
    )
    print()
